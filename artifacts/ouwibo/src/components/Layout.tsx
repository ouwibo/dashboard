    ...
    <div className="px-2 py-2 border-t border-border/40">
      ...
      <div className="flex items-center gap-2 px-2 mb-2">
        ...
        >
          ...
          {expanded && (
            <span className="text-[10px] font-bold" style={{ fontFamily: MONO }}>
              {isDark ? "Dark" : "Light"}
            </span>
          )}
        </button>
    ...
    >
      ...
      {expanded && (
        <>
          ...
          {isStub && !active && (
            <span className="text-[8px] font-bold px-1.5 py-0.5 rounded-full bg-muted text-muted-foreground" style={{ fontFamily: MONO }}>
              SOON
            </span>
          )}
      ...
      <div className="flex-1 overflow-y-auto overflow-x-hidden py-1 space-y-0">
        {NAV_GROUPS.map((group, gi) => (
          <div key={group.label}>
            ...
            {expanded && (
              <p className="px-5 pt-2 pb-1 text-[9px] font-bold text-muted-foreground/40 tracking-widest select-none" style={{ fontFamily: MONO }}>
                {group.label}
              </p>
            )}
      ...
      <header className="fixed top-0 left-0 right-0 z-50 h-13 bg-background/80 backdrop-blur-xl border-b border-border/60 flex items-center gap-2.5 px-3">
        ...
        <Link href="/">
          <div className="flex items-center gap-2 cursor-pointer select-none">
            ...
            <div className="hidden sm:flex flex-col leading-none">
              <span className="font-black text-[11px]" style={{ fontFamily: DISPLAY }}>Ouwibo</span>
              <span className="text-[8px] text-muted-foreground" style={{ fontFamily: MONO }}>Cloud</span>
            </div>
          </div>
        </Link>
        ...
        <div className="flex-1 max-w-sm mx-auto">
          <div className="flex items-center gap-2 bg-muted/40 rounded-lg px-3 py-1.5 text-muted-foreground border border-border/40">
            <Search size={11} />
            <span className="text-[10px]" style={{ fontFamily: MONO }}>Search airdrops, news…</span>
            <span className="ml-auto text-[9px] bg-background border border-border/40 rounded px-1 py-0.5 hidden md:block" style={{ fontFamily: MONO }}>⌘K</span>
          </div>
        </div>
          ...
          return (
            <Link key={item.href} href={item.href}>
              <div className={cn("flex flex-col items-center gap-0.5 px-3 py-1.5 rounded-lg transition-colors", active ? "text-primary" : "text-muted-foreground")}>
                <item.Icon size={18} />
                <span className="text-[8px] font-bold" style={{ fontFamily: MONO }}>{item.label}</span>
              </div>
            </Link>
          );
        })}
