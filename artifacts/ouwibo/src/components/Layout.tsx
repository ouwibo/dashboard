    ...
    <div className="px-2 py-2 border-t border-border/40">
      ...
      <div className="flex items-center gap-2 px-2 mb-2">
        ...
        >
          ...
          {expanded && (
            <span className="text-[10px] font-bold" style={{ fontFamily: "font-sans" }}>
              {isDark ? "Dark" : "Light"}
            </span>
          )}
        </button>
...
function NavLink({
  ...
  const inner = (
    ...
    >
      ...
      {expanded && (
        <>
          <span className="text-[11px] font-bold flex-1 whitespace-nowrap" style={{ fontFamily: "font-sans" }}>
            {item.label}
          </span>
          {isStub && !active && (
            <span className="text-[8px] font-bold px-1.5 py-0.5 rounded-full bg-muted text-muted-foreground" style={{ fontFamily: "font-sans" }}>
              SOON
            </span>
          )}
      ...
      <div className="flex-1 overflow-y-auto overflow-x-hidden py-1 space-y-0">
        {NAV_GROUPS.map((group, gi) => (
          <div key={group.label}>
            ...
            {expanded && (
              <p className="px-5 pt-2 pb-1 text-[9px] font-bold text-muted-foreground/40 tracking-widest select-none" style={{ fontFamily: "font-sans" }}>
                {group.label}
              </p>
            )}
      ...
      <header className="fixed top-0 left-0 right-0 z-50 h-13 bg-background/80 backdrop-blur-xl border-b border-border/60 flex items-center gap-2.5 px-3">
        ...
        <div className="flex-1 max-w-sm mx-auto">
          <div className="flex items-center gap-2 bg-muted/40 rounded-lg px-3 py-1.5 text-muted-foreground border border-border/40">
            <Search size={11} />
            <span className="text-[10px]" style={{ fontFamily: "font-sans" }}>Search airdrops, news…</span>
            <span className="ml-auto text-[9px] bg-background border border-border/40 rounded px-1 py-0.5 hidden md:block" style={{ fontFamily: "font-sans" }}>⌘K</span>
          </div>
        </div>
        ...
        <div className="fixed inset-0 z-50 lg:hidden" onClick={() => setMobileOpen(false)}>
          ...
          >
            <div className="flex items-center justify-between px-4 h-13 border-b border-border/40">
              <span className="font-black text-sm" style={{ fontFamily: "font-sans" }}>Ouwibo Cloud</span>
              <button onClick={() => setMobileOpen(false)} className="w-7 h-7 rounded-lg flex items-center justify-center hover:bg-muted">
                <Menu size={14} />
              </button>
            </div>
        ...
        {ALL_NAV.filter(i => !("soon" in i) || !i.soon).slice(0, 5).map(item => {
          ...
          return (
            <Link key={item.href} href={item.href}>
              <div className={cn("flex flex-col items-center gap-0.5 px-3 py-1.5 rounded-lg transition-colors", active ? "text-primary" : "text-muted-foreground")}>
                <item.Icon size={18} />
                <span className="text-[8px] font-bold" style={{ fontFamily: "font-sans" }}>{item.label}</span>
              </div>
            </Link>
          );
        })}
