...
export function ArticleCard({
  ...
  return (
    <Link
      ...
      className={cn(
        "premium-card premium-card-hover group flex flex-col rounded-2xl border overflow-hidden",
        className,
      )}
    >
      {/* Thumbnail */}
...
export function HeroArticleCard({ article }: { article: Article }) {
  ...
  return (
    <Link
      href={`/article/${article.slug}`}
      className="premium-card premium-card-hover group block rounded-2xl border overflow-hidden"
    >
      {/* Large thumbnail */}
      <div className="h-[220px] sm:h-[260px] w-full overflow-hidden bg-muted relative">
        ...
        <span
          className="absolute top-3 left-3 text-[10px] font-semibold tracking-wide uppercase px-2 py-0.5 rounded-full text-white"
      ...
      {/* Body */}
      <div className="p-4">
        <h2 className="text-[17px] sm:text-[19px] font-bold text-foreground leading-snug group-hover:text-primary transition-colors mb-1.5">
          {article.title}
        </h2>
