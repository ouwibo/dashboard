  ...
  return (
    <Link href={`/article/${article.slug}`}
      ...
      <div className="h-[140px] w-full overflow-hidden bg-muted flex items-center justify-center relative">
        {article.coverImage
          ...
          : <div className="w-full h-full" style={{ background: `linear-gradient(135deg, ${color}22, ${color}44)` }}>
              <span className="absolute inset-0 flex items-center justify-center text-3xl opacity-30 select-none">
      ...
      <div className="p-3 flex flex-col flex-1 gap-1.5">
        <span className="text-[10px] font-semibold tracking-wide uppercase" style={{ color }}>{article.category}</span>
        <h3 className="text-[13px] font-semibold text-foreground leading-snug line-clamp-2 group-hover:text-primary transition-colors">{article.title}</h3>
        <p className="text-[12px] text-muted-foreground leading-relaxed line-clamp-2 flex-1">{article.excerpt}</p>
        <div className="flex items-center justify-between mt-auto pt-1.5 border-t border-border/50">
          <span className="text-[11px] text-muted-foreground font-medium">{article.author}</span>
    ...
    <Link href={`/article/${article.slug}`}
      ...
      <div className="h-[220px] sm:h-[260px] w-full overflow-hidden bg-muted relative">
        ...
        <span className="absolute top-3 left-3 text-[10px] font-semibold tracking-wide uppercase px-2 py-0.5 rounded-full text-white"
          style={{ background: color }}>{article.category}</span>
      </div>
      {/* Body */}
      <div className="p-4">
        <h2 className="text-[17px] sm:text-[19px] font-bold text-foreground leading-snug group-hover:text-primary transition-colors mb-1.5">{article.title}</h2>
