  ...
  return (
    <div className="space-y-8">
          ...
          <div className="flex flex-col gap-4">
            <div className="rounded-xl border border-border bg-card p-4">
              <div className="flex items-center gap-1.5 mb-3">
                ...
                <h3 className="text-[12px] font-semibold uppercase tracking-wide text-muted-foreground">Trending</h3>
              </div>
              <ol className="space-y-3">
                {all.slice(0, 4).map((a, i) => (
                  <li key={a.id}>
                    <Link href={`/article/${a.slug}`} className="flex gap-2.5 group">
                      <span className="text-[20px] font-bold text-border leading-none mt-0.5 flex-shrink-0 w-5">{i + 1}</span>
                      <div>
                        <p className="text-[12px] font-semibold text-foreground leading-snug group-hover:text-primary transition-colors line-clamp-2">{a.title}</p>
                        <p className="text-[11px] text-muted-foreground mt-0.5">{a.readTime}m · {a.category}</p>
                      </div>
                    </Link>
                  </li>
                ))}
            ...
            {/* Ad slot */}
            <AdSlot slot="rectangle" className="hidden lg:flex" />
          </div>
        </div>
      </section>
      ...
      <section>
        <div className="flex items-center justify-between mb-4">
          <div className="flex items-center gap-1.5">
            ...
            <h2 className="text-[15px] font-bold text-foreground">Active Airdrops</h2>
          </div>
          ...
          {airdrops.map((drop) => (
            <Link key={drop.id} href={`/airdrops/${drop.id}`}
              ...
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-1.5">
                  <span className="text-[13px] font-semibold text-foreground">{drop.name}</span>
                  {drop.isVerified && <span className="text-primary text-[10px]">✓</span>}
                </div>
                <p className="text-[11px] text-muted-foreground truncate">{drop.category} · {drop.chain}</p>
              </div>
              {/* Reward */}
              <div className="text-right flex-shrink-0">
                <p className="text-[12px] font-semibold text-primary">{drop.rewardEstimate}</p>
                <p className="text-[10px] text-muted-foreground capitalize">{drop.difficulty}</p>
              </div>
              {/* Status */}
      ...
      <section className="rounded-xl border border-border bg-card p-6 text-center">
        ...
        <p className="text-[13px] text-muted-foreground mb-4">Weekly digest of the best opportunities, analysis, and news.</p>
      ...
      {/* ── Bottom Ad ── */}
      <AdSlot slot="responsive" className="mx-auto" />
    </div>
  );
}
