...
function MiniBar({ score }: { score: number }) {
  ...
  return (
    <div className="relative h-1 w-full rounded-full overflow-hidden mt-1"
      ...
      <div className="absolute top-1/2 -translate-y-1/2 w-1.5 h-1.5 rounded-full bg-white border border-black/20"
           style={{ left: `max(0px, calc(${pct}% - 3px))` }} />
    </div>
  );
}
...
function AirdropRow({ a, rank }: { a: Airdrop; rank: number }) {
  ...
  return (
    <Link href={`/airdrops/${a.slug}`}>
      <div className="flex items-center gap-3 px-4 py-2.5 hover:bg-muted/40 transition-colors cursor-pointer border-b border-border/30 last:border-0">
        <span className="text-[11px] text-muted-foreground w-5 text-center shrink-0">{rank}</span>
        ...
        <div className="flex-1 min-w-0">
          ...
          {task ? (
            <div className="flex items-center gap-1.5 text-[11px] text-muted-foreground">
              <span className={task.cost === 0 ? "text-emerald-500 font-medium" : "text-amber-500 font-medium"}>
        ...
        <div className="text-right shrink-0 hidden sm:block">
          <div className="text-[11px] flex items-center gap-1 justify-end">
            <span>{S_EMOJI[a.status]}</span>
            <span className={cn("font-medium", a.status === "Reward Available" ? "text-blue-400" : "text-foreground")}>
              {a.status}
            </span>
          </div>
        ...
        <div className="shrink-0 w-20 hidden md:block">
          <div className="text-[12px] font-bold text-right">{a.moniScore.toLocaleString()}</div>
          <MiniBar score={a.moniScore} />
        </div>
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="flex items-start justify-between gap-4">
        ...
        <Link href="/airdrops">
          <button className="text-[12px] px-3 py-1.5 bg-primary text-primary-foreground rounded-lg font-medium hover:opacity-90 transition-opacity shrink-0">
            View All →
          </button>
        </Link>
      </div>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
        {[
          ...
          <div key={label} className="border border-border rounded-xl p-3.5 bg-card/60 backdrop-blur-sm">
            <div className={cn("mb-2", cls)}>{icon}</div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
        ...
        <div className="lg:col-span-2 border border-border rounded-xl bg-card/60 backdrop-blur-sm overflow-hidden">
              ...
              <div>
                {rewardA.map((a) => (
                  <Link key={a.id} href={`/airdrops/${a.slug}`}>
                    <div className="flex items-center gap-2.5 px-4 py-2.5 hover:bg-muted/40 transition-colors cursor-pointer border-b border-border/30 last:border-0">
                ...
                {newest.map((a) => (
                  <Link key={a.id} href={`/airdrops/${a.slug}`}>
                    <div className="flex items-center gap-2.5 px-4 py-2.5 hover:bg-muted/40 transition-colors cursor-pointer border-b border-border/30 last:border-0">
                      ...
                      <span className="text-[9px] bg-emerald-500 text-white px-1.5 py-0.5 rounded font-medium shrink-0">NEW</span>
                    </div>
                  </Link>
                ))}
              </div>
