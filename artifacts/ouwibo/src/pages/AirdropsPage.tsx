  ...
  return (
    ...
    >
      {airdrop.logoUrl && !failed ? (
        ...
        <span className="text-[13px] font-black tracking-tight">{airdrop.logoInitial}</span>
      )}
    </div>
  );
}
...
function BackerAvatar({ backer, index }: { backer: Backer; index: number }) {
  ...
  return (
    <div
      ...
      style={{ background: backer.color, marginLeft: index > 0 ? "-7px" : 0, zIndex: 10 - index }}
    >
...
function BackerRow({ backers, extra }: { backers?: Backer[]; extra?: number }) {
  ...
  return (
    <div className="flex items-center justify-end">
      {backers?.slice(0, 4).map((b, i) => <BackerAvatar key={`${b.name}-${i}`} backer={b} index={i} />)}
      {(extra ?? 0) > 0 && <span className="ml-2 text-[11px] font-semibold text-muted-foreground">+{extra}</span>}
    </div>
  );
}
...
function StatusBadge({ status }: { status: Airdrop["status"] }) {
  return (
    <span className={cn("inline-flex h-7 items-center gap-1.5 rounded-full border px-2.5 text-[11px] font-bold", STATUS_BADGE[status])}>
      {STATUS_ICON[status]}
      {status}
    </span>
  );
  ...
  return (
    <div className="flex min-w-0 flex-wrap items-center gap-1.5">
      {types.slice(0, max).map((type) => (
        <span key={type} className={cn("rounded-md border px-2 py-1 text-[10px] font-bold leading-none", TYPE_CLS[type] ?? "border-border bg-muted text-muted-foreground")}>
          {type}
        </span>
      ))}
      {types.length > max && <span className="text-[10px] font-black text-primary">+{types.length - max}</span>}
    </div>
  );
}
...
function AirdropRow({ a, bookmarked, onToggle }: { a: Airdrop; bookmarked: boolean; onToggle: () => void }) {
  ...
  return (
    <Link href={`/airdrops/${a.slug}`} className="group block">
      <article className="grid min-h-[92px] grid-cols-1 gap-4 rounded-2xl border border-border/60 bg-card/95 p-4 shadow-sm transition-all duration-200 hover:-translate-y-0.5 hover:border-primary/30 hover:bg-card hover:shadow-lg sm:grid-cols-[minmax(220px,1.15fr)_120px_minmax(220px,1fr)_130px_44px] sm:items-center lg:grid-cols-[minmax(260px,1.25fr)_130px_minmax(280px,1fr)_150px_44px]">
        <div className="flex min-w-0 items-center gap-3">
          <ProjectLogo airdrop={a} />
          <div className="min-w-0 space-y-2">
            <div className="flex min-w-0 items-center gap-2">
              <h3 className="truncate text-[14px] font-black leading-tight tracking-tight text-foreground group-hover:text-primary">{a.name}</h3>
              {a.isNew && <span className="rounded-full bg-emerald-500 px-2 py-0.5 text-[9px] font-black uppercase leading-none text-white">New</span>}
            </div>
            <div className="flex min-w-0 flex-wrap items-center gap-1.5">
        ...
        <div className="grid grid-cols-2 gap-2 sm:grid-cols-1 sm:gap-1 sm:border-l sm:border-border/40 sm:pl-4">
          <div className="text-[11px] font-medium uppercase tracking-[0.18em] text-muted-foreground sm:hidden">Cost / time</div>
          <div className="flex items-center gap-3 sm:block">
            <p className={cn("text-[15px] font-black leading-none", totalCost === 0 ? "text-emerald-400" : "text-amber-400")}>
        ...
        <div className="min-w-0 space-y-2">
          <p className="truncate text-[13px] font-bold leading-tight text-foreground">{topTask?.name ?? "No active tasks"}</p>
          {topTask ? <TypePills types={topTask.types} /> : <span className="text-[11px] text-muted-foreground">No requirement listed</span>}
          {a.tasks.length > 1 && <p className="text-[11px] font-semibold text-muted-foreground">+{a.tasks.length - 1} more task{a.tasks.length > 2 ? "s" : ""}</p>}
        </div>

        <div className="flex items-center justify-between gap-3 sm:block sm:text-right">
          <div>
            <p className="text-[10px] font-semibold uppercase tracking-[0.18em] text-muted-foreground">Funding</p>
            <p className="mt-1 text-[12px] font-black text-foreground">{a.raiseFunds ?? "—"}</p>
          </div>
        ...
        <div className="flex items-center justify-between border-t border-border/40 pt-3 sm:block sm:border-t-0 sm:pt-0 sm:text-right">
          <button
            ...
            className={cn(
              ...
              bookmarked ? "border-amber-400/20 bg-amber-400/10 text-amber-400" : "border-border/60 bg-background/50 text-muted-foreground hover:text-amber-300",
            )}
...
function AirdropCard({ a, bookmarked, onToggle }: { a: Airdrop; bookmarked: boolean; onToggle: () => void }) {
  ...
  return (
    <Link href={`/airdrops/${a.slug}`} className="group block h-full">
      <article className="flex h-full min-h-[320px] flex-col rounded-2xl border border-border/60 bg-card p-4 shadow-sm transition-all duration-200 hover:-translate-y-1 hover:border-primary/30 hover:shadow-xl">
        <div className="flex items-start justify-between gap-3">
          <div className="flex min-w-0 items-center gap-3">
            <ProjectLogo airdrop={a} />
            <div className="min-w-0">
              <div className="flex min-w-0 items-center gap-2">
                <h3 className="truncate text-[15px] font-black leading-tight group-hover:text-primary">{a.name}</h3>
                {a.isNew && <span className="rounded-full bg-emerald-500 px-2 py-0.5 text-[9px] font-black uppercase leading-none text-white">New</span>}
              </div>
              {a.ticker && <p className="mt-1 text-[11px] font-semibold text-muted-foreground">{a.ticker}</p>}
            </div>
          </div>
          <button
            onClick={(event) => {
            ...
            className={cn("grid h-9 w-9 shrink-0 place-items-center rounded-xl border transition-all", bookmarked ? "border-amber-400/20 bg-amber-400/10 text-amber-400" : "border-border/60 bg-background/50 text-muted-foreground")}
            aria-label={bookmarked ? "Remove bookmark" : "Bookmark airdrop"}
          >
            <Star className="h-4 w-4" fill={bookmarked ? "currentColor" : "none"} />
          </button>
        ...
        <div className="mt-4 grid grid-cols-3 gap-2 rounded-2xl border border-border/50 bg-background/45 p-3">
          <div>
            ...
            <p className={cn("mt-1 text-[13px] font-black", totalCost === 0 ? "text-emerald-400" : "text-amber-400")}>{totalCost === 0 ? "Free" : `$${totalCost}`}</p>
          </div>
          <div>
            <p className="text-[10px] font-semibold uppercase tracking-wider text-muted-foreground">Time</p>
            <p className="mt-1 text-[13px] font-black text-foreground">{totalTime}m</p>
          </div>
          <div>
            <p className="text-[10px] font-semibold uppercase tracking-wider text-muted-foreground">Tasks</p>
            <p className="mt-1 text-[13px] font-black text-foreground">{a.tasks.length}</p>
          </div>
        </div>

        <div className="mt-4 min-w-0 flex-1 space-y-2">
          <p className="truncate text-[13px] font-bold">{topTask?.name ?? "No active tasks"}</p>
          {topTask ? <TypePills types={topTask.types} max={2} /> : <span className="text-[11px] text-muted-foreground">No requirement listed</span>}
        </div>
        ...
        <div className="mt-5 border-t border-border/50 pt-4">
          ...
          <div className="mt-4 flex items-center justify-between text-[12px] font-black text-primary">
            ...
            <span className="inline-flex items-center gap-1">View Details <ChevronRight className="h-3.5 w-3.5" /></span>
          </div>
        </div>
      </article>
    </Link>
...
export default function AirdropsPage() {
  ...
  const filtered = useMemo(
    () =>
      mockAirdrops.filter((airdrop) => {
        ...
        if (rewardTab !== "All" && airdrop.rewardType !== rewardTab) return false;
        if (query && !`${airdrop.name} ${airdrop.ticker ?? ""}`.toLowerCase().includes(query)) return false;
        return true;
      }),
  ...
  return (
    <div className="mx-auto w-full max-w-[1320px] space-y-5 pb-8">
      <section className="rounded-3xl border border-border/60 bg-card/70 p-4 shadow-sm backdrop-blur sm:p-5">
        <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
          <div className="min-w-0">
            ...
            <h1 className="mt-1 text-2xl font-black tracking-tight text-foreground sm:text-3xl">Find campaigns worth farming</h1>
            <p className="mt-2 max-w-2xl text-sm leading-6 text-muted-foreground">
              Clean list of active, potential, and reward-ready campaigns with cost, time, requirements, and funding signals aligned for fast scanning.
            </p>
          </div>
      ...
      <section className="rounded-3xl border border-border/60 bg-card/70 p-3 shadow-sm backdrop-blur sm:p-4">
            ...
            {FILTER_TABS.map((item) => (
              <button
                ...
                className={cn(
                  ...
                  tab === item ? "border-primary bg-primary text-primary-foreground shadow-sm" : "border-border/60 bg-background/45 text-muted-foreground hover:text-foreground",
                )}
              >
                {item}
              </button>
          ...
          <div className="flex items-center gap-2">
            ...
            <div className="hidden items-center gap-1 rounded-full border border-border/60 bg-background/55 p-1 lg:flex">
              <button
                ...
                className={cn("grid h-8 w-8 place-items-center rounded-full transition-colors", view === "list" ? "bg-primary text-primary-foreground" : "text-muted-foreground hover:text-foreground")}
              ...
              <button
                ...
                className={cn("grid h-8 w-8 place-items-center rounded-full transition-colors", view === "cards" ? "bg-primary text-primary-foreground" : "text-muted-foreground hover:text-foreground")}
          ...
          {REWARD_TABS.map((item) => (
            <button
              ...
              className={cn(
                ...
                rewardTab === item ? "bg-foreground text-background" : "bg-background/45 text-muted-foreground hover:text-foreground",
              )}
            >
              {item}
            </button>
